...
class UserControllerIT {
    ...
    @BeforeEach
    void setUp() {
        this.testManager.postgresql().clearAllData(List.of(DatabaseContract.USER_ENTITY_DB_CONTRACT));
    }
    ...
    void givenInvalidRequest_whenRegisterUser_thenBadRequestAndNoUserPersisted(
        ...
        //then
        final List<UserEntity> users = this.testManager.postgresql().get(UserEntity.class).getAll();
        assertEquals(0, users.size(), "No user must be persisted for case: " + testCase);
    }
    ...
    @Test
    @DisplayName("Should reject registration when email already exists (even for different siteId)")
    void givenExistingEmailDifferentSiteId_whenRegisterUser_thenRejectRegistration() {
        //given
